...
import type { Answers, AppData, DiagnosticResults } from './models/types.js';
...
class LearningApp {
   ...
   */
  private computeAndShowResults(): void {
    if (!this.appData) return;
   ...
   */
  private handleExport(results: DiagnosticResults): void {
    try {
   ...
   */
  private async handleCopyPrompt(results: DiagnosticResults): Promise<void> {
    try {
